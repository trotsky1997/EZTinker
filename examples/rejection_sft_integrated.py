...
def rejection_collect_data(
    model, tokenizer, dataset: SimpleMathDataset, max_generations_per_sample: int = 10
    ...
    print("收集结果:")
    print(
        f"  成功率: {stats['collected']}/{stats['total_samples']} = {stats['collected'] / stats['total_samples'] * 100:.1f}%"
    )
...
def train_on_collected_data(
    model, tokenizer, training_samples: list[dict], output_dir: str = "./rejection_output"
):
    """在筛选后的数据上训练模型"""
...
def evaluate_model(model, tokenizer, eval_dataset: SimpleMathDataset, max_samples: int = 20):
    """评估训练好的模型"""
